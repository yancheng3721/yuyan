...
$(function() {
	...
	var allFiledsStr = '[{"_CN_":"模块名","_KEY_":"NAME","_NOT_NULL_":"1"},{"_CN_":"模块编码","_KEY_":"CODE","_NOT_NULL_":"1"},{"_CN_":"ID","_KEY_":"ID"},{"_CN_":"更新用户","_KEY_":"UPDATE_USER"},{"_CN_":"创建时间","_KEY_":"CREATE_TIME"},{"_CN_":"更新时间","_KEY_":"UPDATE_TIME"}]';
    ...
    function expand(obj){
    	if(obj.ID){
			$.ajax({
				url: '/yuyan/module/expandModule.do',
				type: 'POST',
				dataType: 'json',
				timeout: 5000,
				data:obj,
	...
	function doBtchDel(){
		...
		$.ajax({
				url: '/yuyan/module/batchDeleteModule.do',
				type: 'POST',
				dataType: 'json',
				timeout: 5000,
				data:{ids:ids},
	...
	function doSave(){
		...
		//alert(222);
		$.ajax({
				url: '/yuyan/module/saveModule.do',
				type: 'post',
				dataType: 'json',
				timeout: 5000,
				data:params,
	...
	function doDelete(obj){
		...
		$.ajax({
				url: '/yuyan/module/deleteModule.do',
				type: 'post',
				dataType: 'json',
				timeout: 5000,
				data:obj,
	...
	function doQuery(){
		...
		searchForm.attr("action","/yuyan/module/queryModule.do");
		searchForm.submit();
	}
	...
	function syncData(){
		...
		$.ajax({
				url: '/yuyan/module/syncModule.do',
				type: 'POST',
				dataType: 'json',
		...
		$('#sguploadForm').ajaxSubmit({
                ...
                success:function(data){
                	...
                	if (data.obj.message == "上传成功!") {
                	   window.location.href="/yuyan/module/manageModule.do";
                	}
                },
                error:function(xhr){
	...
	function exportCsv() {
		var searchForm = $("#searchForm");
		var tmp = searchForm.attr("action");
		searchForm.attr("action","/yuyan/module/exportModule.do");
		searchForm.submit();
...
<input name='searchbox.CODE' type='text' id='searchbox.CODE'  value='${searchbox["CODE"]}' />
			...
			<div class="searchCenter">
				<input type="hidden" name="exportFields" value="NAME,CODE"/>
...
<td>${obj.UPDATE_USER}</td>
			...
			<div class="searchCenter pagenation" style="text-align:right">
				<script type="text/javascript">
					window.__page=new SnPage(document.getElementById("searchForm"),'/yuyan/module/queryModule.do','${page.totalPage?c}','${page.currentPage?c}');
				</script>
...
<tr style='height:35px;'>
<td width='140px' align='right'>模块编码：</td>
...
<div class="showBox" id="batchupload" style="display:none;" title="模块批量上传">
	<form action="/yuyan/module/uploadModule.do" method="POST" id="sguploadForm" name="sguploadForm" enctype="multipart/form-data" onsubmit="return false;">
		<div style="text-align: center;margin: 10px 0;">
			<span>路径：</span>
			<input type="hidden" name="importFields" value="NAME,CODE"/>
