...
$(function() {
    ...
    function expand(obj){
    	if(obj.ID){
			$.ajax({
				url: '/admin/resource/expandResource.do',
				type: 'POST',
				dataType: 'json',
				timeout: 5000,
				data:obj,
	...
	function doBtchDel(){
		...
		$.ajax({
				url: '/admin/resource/batchDeleteResource.do',
				type: 'POST',
				dataType: 'json',
				timeout: 5000,
				data:{ids:ids},
	...
	function doSave(){
		...
		//alert(222);
		$.ajax({
				url: '/admin/resource/saveResource.do',
				type: 'post',
				dataType: 'json',
				timeout: 5000,
				data:params,
	...
	function doDelete(obj){
		...
		$.ajax({
				url: '/admin/resource/deleteResource.do',
				type: 'post',
				dataType: 'json',
				timeout: 5000,
				data:obj,
	...
	function doQuery(){
		...
		searchForm.attr("action","/admin/resource/queryResource.do");
		searchForm.submit();
	}
	...
	function syncData(){
		...
		$.ajax({
				url: '/admin/resource/syncResource.do',
				type: 'POST',
				dataType: 'json',
		...
		$('#sguploadForm').ajaxSubmit({
                ...
                success:function(data){
                	...
                	if (data.obj.message == "上传成功!") {
                	   window.location.href="/admin/resource/manageResource.do";
                	}
                },
                error:function(xhr){
	...
	function exportCsv() {
		var searchForm = $("#searchForm");
		var tmp = searchForm.attr("action");
		searchForm.attr("action","/admin/resource/exportResource.do");
		searchForm.submit();
...
<td>${obj.UPDATE_USER}</td>
			...
			<div class="searchCenter pagenation" style="text-align:right">
				<script type="text/javascript">
					window.__page=new SnPage(document.getElementById("searchForm"),'/admin/resource/queryResource.do','${page.totalPage?c}','${page.currentPage?c}');
				</script>
...
<div class="showBox" id="batchupload" style="display:none;" title="资源批量上传">
	<form action="/admin/resource/uploadResource.do" method="POST" id="sguploadForm" name="sguploadForm" enctype="multipart/form-data" onsubmit="return false;">
